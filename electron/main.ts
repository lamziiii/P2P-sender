...
try { config.downloadPath = app.getPath('downloads'); } catch (e) { }
if (fs.existsSync(configFile)) {
  try {
    const c = JSON.parse(fs.readFileSync(configFile, 'utf8'));
    if (c.downloadPath) config.downloadPath = c.downloadPath;
    if (c.autoLaunch !== undefined) config.autoLaunch = c.autoLaunch;
  } catch (e) { }
} else {
...
let friends: { id: string, name: string, online: boolean }[] = [];
...
let messages: Record<string, { id: string, from: string, text: string, ts: number }[]> = {};
...
const joinChatWithFriend = (friendId: string) => {
...
for (const f of friends) joinChatWithFriend(f.id);

// Presence tracking
const onlineFriends = new Set<string>();
const setFriendOnline = (friendId: string, online: boolean) => {
  const before = onlineFriends.has(friendId);
  if (online) onlineFriends.add(friendId); else onlineFriends.delete(friendId);
  if (before !== online) {
...
chatSwarm.on('connection', (socket: any) => {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        if (msg.type === 'ping' && msg.from) {
          ...
          if (isRateLimited(msg.from)) continue;
  ...
  socket.on('close', () => {
    if (peerId) setFriendOnline(peerId, false);
  });
});

// Chat IPC
ipcMain.handle('get-messages', (_e, friendId: string) => messages[friendId] ?? []);
...
ipcMain.handle('send-message', (_e, { friendId, text }: { friendId: string, text: string }) => {
  // Guard outbound messages
  if (isRateLimited(`out:${friendId}`)) return { error: 'rate_limited' };
  ...
  for (const conn of (chatSwarm as any).connections) {
...
swarm.on('connection', (socket: any) => {
  socket.on('error', () => { /* ignore random disconnects */ });
        ...
        try {
          ...
          if (header.type === 'file_offer') {
            ...
            if (window) {
              ...
              const accept: boolean = await new Promise((resolve) => {
                ipcMain.once(`file-offer-response-${offerId}`, (_e, accepted: boolean) => resolve(accepted));
              ...
              if (accept) {
                ...
                writeStream.on('finish', () => {
                  if (Notification.isSupported()) {
                    ...
                    notif.on('click', () => {
                      if (window) { window.show(); window.focus(); window.webContents.send('switch-tab', 'transfers'); }
                    });
                    notif.show();
                  }
                });
...
ipcMain.handle('send-file', async (_e, friendId) => {
  ...
  transferSwarm.on('connection', (socket: any) => {
    socket.on('error', () => { /* ignore random disconnects */ });
    ...
    socket.once('data', (data: Buffer) => {
      ...
      if (accept) {
        ...
        readStream.on('data', (chunk: Buffer | string) => {
          progress += (chunk as Buffer).length;
